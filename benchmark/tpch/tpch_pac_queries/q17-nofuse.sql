...
-- the pac_select ANDs the 64 booleans makes a probabilistic choice over 64 booleans
...
SELECT pac_noised_sum(pac_pu, l_extendedprice) / 7.0 AS avg_yearly
  FROM (SELECT pac_select(
                 pac_hash(hash(o_custkey)),
                 list_transform(
                   list_transform(
                     (SELECT list_transform(
                               list_zip(pac_sum(pac_hash(hash(o_sub.o_custkey)), l_sub.l_quantity),
