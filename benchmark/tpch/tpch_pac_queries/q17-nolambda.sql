SELECT pac_noised_sum(pac_hash(hash(o_custkey)), l_extendedprice) / 7.0 AS avg_yearly
...
WHERE part.p_brand = 'Brand#23'
  ...
  AND lineitem.l_quantity < (SELECT 0.2 * (pac_noised_sum(pac_hash(hash(o_sub.o_custkey)), l_quantity) / 
                                           pac_noised_count(pac_hash(hash(o_sub.o_custkey)), l_quantity))
