SELECT l_returnflag, l_linestatus,
       ...
       pac_sum(hash(orders.o_custkey), l_quantity) / count(l_quantity) AS avg_qty,
       pac_sum(hash(orders.o_custkey), l_extendedprice) / count(l_extendedprice) AS avg_price,
       pac_sum(hash(orders.o_custkey), l_discount) / count(l_discount) AS avg_disc,
